...
class BuildGraph:
    ...
    def save(self):
        ...
        nx.write_weighted_edgelist(self.g,
                                   f"{self.graph_path}/{self.dataset}+graph.txt")
        #         import matplotlib.pyplot as plt
        # 可视化
        #         plt.subplots(1, 1, figsize=(50, 50))
        #         nx.draw(self.g, with_labels=True, font_weight='normal',font_family='SimHei')
        #         plt.axis('on')
        #         plt.xticks([])
        #         plt.yticks([])
        #         plt.show()
        G = self.g
        dataset="0_1_5850275.clean"
        with open(f"{dataset}_net.pkl", 'wb') as f:
            pickle.dump(G, f)